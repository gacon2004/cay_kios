  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      const response = await api.get(
        `/appointments/${appointment.id}/print?has_insurances=true`,
        {
          responseType: 'blob', // Quan trọng: Đặt responseType là blob để xử lý file PDF
        }
      );
      ...
      // Chờ iframe load hoàn tất rồi in
      iframe.onload = () => {
        try {
          ...
          // Giải phóng URL object sau khi in
      ...
      // Giải phóng URL sau khi mở tab
      setTimeout(() => {
        window.URL.revokeObjectURL(pdfUrl);
      }, 1000); // Delay để đảm bảo tab mới load xong
