...
    // LẤY GIÁ TRỊ HAS_INSURANCE TỪ LOCALSTORAGE
    const hasInsurance = localStorage.getItem("has_insurance") === "true";
...
    try {
      const data = await createAppointment(dataToSend, token, hasInsurance);
      setResult(data);
    } catch (err) {
...
        `http://localhost:8000/appointments/${appointmentId}/print?insurances=${hasInsurance}`,
        {
          headers: {
...
              <p>Phòng: {result.clinic_name}</p>
              <p>Bác sĩ: {result.doctor_name}</p>
              <p>Số thứ tự: {result.queue_number}</p>
              <p>Thời gian: {new Date(result.appointment_time).toLocaleString("vi-VN")}</p>
            </div>
          </div>
